    ...
    def write_planning(self):
        if len(self.beacon_positions) > 1:
            ...
            with open(self.pathfile, 'w') as planfile:
                for i in [i+1 for i in range(len(self.beacon_positions)-1)]: 
                    self.beacon_path = SearchTree(SearchProblem(Domain(self.map_connections), self.beacon_positions[i-1], self.beacon_positions[i]), 'a_star').search(20000)[0]
                    ...
                    if i+1 == int(self.nBeacons):
                        self.beacon_path = SearchTree(SearchProblem(Domain(self.map_connections), self.beacon_positions[i], self.beacon_positions[0]), 'a_star').search(20000)[0]
                        for idx, cell in enumerate(self.beacon_path):
                            planfile.write(  '\n' + ''.join(str(cell[0])) + ' ' + ''.join(str(cell[1])))
